...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'D:/project8-master/my-project-server/images'); // create an 'uploads' directory
    },
...
app.post('/api/uploads', upload.single('image'), async (req, res) => {
    ...
    try {
        var result = await Order.createReceipt(pool, 
...
app.get("/api/orderlist/:userId", async (req, res) => {
    const userId = req.params.userId;
    console.log(result);
    console.log(user.id);
    try {
        var result = await Order.getAllOrderByuser(pool, userId);
...
app.get('/api/images/:imageName', (req, res) => {
    ...
    fs.readFile(imagePath, (err, data) => {
      if (err) {
        ...
        res.contentType('image/jpeg'); // Set the content type based on the image type (e.g., image/jpeg)
        res.send(data);
      }
    });
  });
